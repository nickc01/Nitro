...
public class CarMovement : MonoBehaviour
{
	...
	public NitroVariable<float> Acceleration = new NitroVariable<float>(30f);
	[SerializeField]
	[Tooltip("The maximum velocity the car can travel")]
	public NitroVariable<float> TerminalVelocity = new NitroVariable<float>(8f);

	[SerializeField]
	[Tooltip("How fast the car rotates in degrees per second")]
	public NitroVariable<float> RotationSpeed = new NitroVariable<float>(90f);
	...
	private void FixedUpdate()
	{
		...
		{
			var accel = transform.forward * Input.GetAxis("Vertical") * Acceleration * Time.fixedDeltaTime;
