...
public class RoadPiece : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        foreach (var localPos in GetOccupiedLocalPositions())
        {
            ...
            //Debug.DrawLine(worldPos, worldPos + new Vector3(0f, 0.1f, 0f), Color.yellow);
            pointCount++;
        }
        ...
        {
            //var worldPos = transform.TransformPoint(localPos);
            //Debug.DrawLine(worldPos + new float3(0f, 0.1f, 0f), worldPos + new float3(0f, 0.2f, 0f), Color.green);
            pointCount++;
        }
    ...
    public IEnumerable<InOutPair> GetInOutPairs()
    {
        ...
        {
            yield return new InOutPair(transform.GetChild(i),transform.GetChild(i + 1));
        }*/
    }
    ...
    public IEnumerable<float3> GetOccupiedLocalPositions()
    {
        ...
        for (float x = bounds.min.x; x <= bounds.max.x; x += SourceGenerator.TileSize.x)
        {
            for (float y = bounds.min.y; y <= bounds.max.y; y += SourceGenerator.TileSize.y)
            {
                ...
                {
                    //Debug.Log("Local Point = " + new float3(x, y, z));
